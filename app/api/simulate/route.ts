...
import { generateKnockoutIfNeeded, progressKnockout, recomputeStandings, simulateMatch } from '@/lib/tournament';

const schema = z.object({ matchId: z.string().optional(), all: z.boolean().optional() });
...
export async function POST(req: NextRequest) {
  ...
  if (body.data.all) {
    ...
    for (const match of matches) await simulateMatch(match.id);
  } else if (body.data.matchId) {
    await simulateMatch(body.data.matchId);
  }
